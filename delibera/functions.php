...
function get_delibera_header() {
	...
	<header class="clearfix">
		...
		<div class="alignright">
			<a class="btn"
			   href="<?php echo get_page_link( get_page_by_slug( DELIBERA_ABOUT_PAGE )->ID ); ?>"><?php _e( 'Saiba por que e como participar', 'delibera' ); ?></a>
		</div>
	</header>
...
function delibera_gerar_curtir( $ID, $type = 'pauta' ) {
	...
	if ( delibera_current_user_can_participate() ) {
		...
		if ( ! delibera_ja_curtiu( $ID, $user_id, $ip, $type ) && // Ainda não curitu
		     ...
		     ! ( delibera_ja_discordou( $ID, $user_id, $ip, $type ) )
		) // não discordou
		{
...
function delibera_gerar_discordar( $ID, $type = 'pauta' ) {
	...
	if ( delibera_current_user_can_participate() ) {
		...
		if ( ! delibera_ja_discordou( $ID, $user_id, $ip, $type ) && // Ainda não curitu
		     ( is_object( $situacao ) && array_key_exists( $situacao->slug, $situacoes_validas ) ) && $situacoes_validas[ $situacao->slug ] &&// é uma situação válida
		     ! ( delibera_ja_curtiu( $ID, $user_id, $ip, $type ) )
		) // não discordou
		{
			$html = '';
...
function delibera_tema_edit_meta_field( $tema ) {
	...
	<tr class="form-field">
		...
		<td>
			<textarea name="tema_meta[excerpt]"
			          id="tema_meta[excerpt]"><?php echo esc_attr( $tema_meta['excerpt'] ) ? esc_attr( $tema_meta['excerpt'] ) : ''; ?></textarea>
